      ...
      <div className="container-fluid mt-3">
        <div>
          ...
          <BrowserRouter>
            <div>
              {token && (
                <LogoutButton onLogout={handleLogout} />  // Utiliza el componente LogoutButton
              )}
            </div>
            <Routes>
              <Route path="/loginAdmin" element={<LoginAdminForm onLogin={handleLogin} titulo="LoginAdminForm" />} />
              <Route path="/registerAdmin" element={<ProtectedRoute token={token} element={<RegisterAdminForm onRegister={handleLogin} />} />} /> {/* Nueva ruta de registro */}
